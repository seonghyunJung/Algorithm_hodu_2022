...
if __name__ == '__main__':
    ...
    while len(order) > 0:
        for b in order:
            ...
            # 놀고있는 기계가 있다면, 놀고있는 기계의 타입에 맞는 작업을 먼저 수행한다.
            if machineController.find(0) == True:
                if b.getCurrentStep().actType == 1:
                    res = person.do(b)
                for bb in order:
                    # 기계가 꽉 찼고, 그 기계를 사용하는 작업만이 남은 경우에는 대기해야 한다. ex) 아메리카노 3잔주문
                    if not((bb.getCurrentStep().actType == 0) or ((bb.getCurrentStep().actType == 2) and (machineController.find(1)))):
                        f1 = machineController.getEarlistEnd(1)
                        f2 = machineController.getEarlistEnd(2)
                        f = min(f1, f2)
                        person.timeFlow(f)
            ...
            if machineController.find(1) == True:
                if b.getCurrentStep().actType == 2:
                    res = person.do(b)
                for bb in order:
                    # 기계가 꽉 찼고, 그 기계를 사용하는 작업만이 남은 경우에는 대기해야 한다. ex) 아메리카노 3잔주문
                    if not((bb.getCurrentStep().actType == 0) or ((bb.getCurrentStep().actType == 1) and (machineController.find(0)))):
                        f1 = machineController.getEarlistEnd(1)
                        f2 = machineController.getEarlistEnd(2)
                        f = min(f1, f2)
                        person.timeFlow(f)
            ...
            # 놀고있는 기계가 없으면, 사람이 하는 작업(type 0)을 한다.
            res = person.do(b)
            if res != 0:
                if res == 2:
                    order.remove(b)
...
# 1. 기계를 최대한 효율적으로 돌려서 기계가 돌아가는 동안 기다리는 것을 최소화한다.
# 2. 기계가 꽉 찼고, 그 기계를 사용하는 작업만이 남은 경우에는 대기해야 한다.